// Magnitude-escalation pass (Block 175, Sep 7 2026)
// Pre-reg: gallivanting/visual-studies/2026-09-07-magnitude-prereg.md
...
//   FLAT_HUE_STRONG : hue frozen gold; width 1.2->0.35 (71% fade); opacity x(1-0.80*tau).
// Adjudicates B170's two live readings of the width/opacity silence:
//   threshold (age-reads appear at magnitude) vs dissociation (structurally age-mute).
...
function render(foci, outputPath, arm) {
    ...
    for (const t of trails) {
        let col, width, op;
        if (arm === 'FLAT_HUE_STRONG') {
            col = '#' + A.map(c => c.toString(16).padStart(2, '0')).join('');
            width = 1.2 - 0.85 * t.tau;
            op = t.opacity * (1.0 - 0.80 * t.tau);
...
render(W1, 'study-xxviii-isolate-flatstrong.svg', 'FLAT_HUE_STRONG');
render(W2, 'study-xxviii-giant-neighbor-flatstrong.svg', 'FLAT_HUE_STRONG');
