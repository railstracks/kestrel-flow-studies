// DIM_TERMINAL pass (Block 180, Sep 8 2026)
// Pre-reg: gallivanting/visual-studies/2026-09-08-dimterminal-prereg.md
...
//   DIM_TERMINAL : EXACT HUE_ONLY chroma trajectory (gold->smoke lerp by tau),
//                  then uniform RGB scaling x(1 - 0.60*tau)  [terminal 55,59,64].
//                  Width frozen 1.2, opacity = t.opacity (as HUE_ONLY).
//   Uniform RGB scaling preserves hue+saturation exactly (HSV S scale-invariant):
//   the ONLY changed variable vs the committed HUE_ONLY anchor is ink luminance.
// Adjudicates the arc's last edge: does the aging-read survive a luminance collapse?
...
function render(foci, outputPath, arm) {
    ...
    for (let k = 0; k < COHORTS; k++) {
        ...
        for (let i = 0; i < TPC; i++) {
            ...
            const opacity = 0.35 + rand() * 0.40; // SAME rand() consumption as original; no tau fade in HUE_ONLY-family arms
            const pts = [[x, y]];
    ...
    for (const t of trails) {
        let col, width, op;
        if (arm === 'DIM_TERMINAL') {
            const hueOnly = [0, 1, 2].map(c => lerp(A[c], B[c], t.tau));
            const dim = hueOnly.map(c => c * (1.0 - 0.60 * t.tau));
            col = '#' + dim.map(c => Math.round(c).toString(16).padStart(2, '0')).join('');
            width = 1.2;
            op = t.opacity;
    ...
    console.log(outputPath, 'trails:', trails.length, 'bytes:', svg.length, 'arm:', arm);
}
const W1 = [{ x: 42, y: -40, g: 38, k: 5.01 }];
const W2 = [{ x: 42, y: -40, g: 38, k: 5.01 }, { x: -52, y: 38, g: 90, k: 1.00 }];
render(W1, 'study-xxviii-isolate-dimterminal.svg', 'DIM_TERMINAL');
render(W2, 'study-xxviii-giant-neighbor-dimterminal.svg', 'DIM_TERMINAL');
